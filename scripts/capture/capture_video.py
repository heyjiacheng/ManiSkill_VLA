"""Capture multi-view images, end-effector trajectory, and generate videos for a ManiSkill task."""
...
class Args:
    ...
    save_video: Annotated[bool, tyro.conf.arg(aliases=["--video"])] = True
    video_fps: int = 10  # Frames per second for generated videos
...
def _generate_videos(destination: Path, camera_uids: List[str], fps: int = 10) -> None:
    """Generate videos from captured images for each camera view.

    Args:
        destination: Root directory containing captured images
        camera_uids: List of camera view identifiers
        fps: Frames per second for the output video
    """
    images_dir = destination / "images"
    if not images_dir.exists():
        print("Warning: No images directory found. Skipping video generation.")
        return

    video_dir = destination / "videos"
    video_dir.mkdir(exist_ok=True)

    # Get all step directories sorted by step number
    step_dirs = sorted(images_dir.glob("step_*"))
    if not step_dirs:
        print("Warning: No image steps found. Skipping video generation.")
        return

    print(f"\nGenerating videos from {len(step_dirs)} frames...")

    # Create a video for each camera view
    for camera_uid in camera_uids:
        frames = []
        for step_dir in step_dirs:
            image_path = step_dir / f"{camera_uid}.png"
            if image_path.exists():
                img = imageio.imread(image_path)
                frames.append(img)

        if frames:
            video_path = video_dir / f"{camera_uid}.mp4"
            imageio.mimwrite(video_path, frames, fps=fps, codec="libx264", quality=8)
            print(f"  Saved video: {camera_uid}.mp4 ({len(frames)} frames)")
        else:
            print(f"  Warning: No frames found for camera '{camera_uid}'")
...
def main(args: Args) -> None:
    ...
    if args.save_video:
        print(f"Mode: Video recording enabled (FPS: {args.video_fps})")
    ...
    try:
        ...
        if args.save_video:
            camera_uids = [uid for uid, _, _ in CAMERA_VIEWS]
            _generate_videos(run_root, camera_uids, fps=args.video_fps)
        ...
        if args.save_video:
            print(f"Videos: {run_root / 'videos'}")
